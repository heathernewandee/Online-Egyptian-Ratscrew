...
public class ColorPanel extends JPanel {
	...
	public void drawPlay(Graphics g) {
		...
		if (turn == 2) {
			if (playerTwo.getHand().getDeck().size() == 0) {
				turn = 1;
				return;
			}
			middle.draw(g);
			computerSlap(playerTwo);
		...
		// when a player wins
		if (playerOne != null && playerOne.getHand().getDeck().size() == 52 || playerTwo.getHand().getDeck().size()==0)
			drawEnd(g, 1);
		if (playerTwo != null && playerTwo.getHand().getDeck().size() == 52 || playerOne.getHand().getDeck().size()==0)
			drawEnd(g, 2);
	...
	public void computerSlap(Computer p) {
		long slap = (long)p.computerSlap(middle, doobleDraw, sandwichDraw, addTo10Draw, threeCardRunDraw, red10Draw, sandwich10Draw, bottomsDraw, marriageDraw);
		if (slap < 0) {
			turn = 1;
			return;
		}
...
//				g.drawString("OOPS! BURNING ONE!", 410, 440);
	...
	private class PanelListener extends MouseAdapter {
		public void mousePressed(MouseEvent e) {
			...
			if ((activeScreen == 7 || activeScreen == 1) && exit.checkIfInsideButton(clickX, clickY)) {
				activeScreen = 0;
			}
			if (activeScreen == 1 && playAgain.checkIfInsideButton(clickX, clickY)) {
				activeScreen = 1;
				active = false;
				didDeal = false;
			}
	...
	private class Controls implements KeyListener {
		...
		public void keyPressed(KeyEvent e) {
			...
			if (e.getKeyCode() == KeyEvent.VK_ENTER && turn == 1) {
				if (playerOne.getHand().getDeck().size() > 1)
